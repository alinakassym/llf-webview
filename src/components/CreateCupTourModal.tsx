// llf-webview/src/components/CreateCupTourModal.tsx
...
import type { CupGroupTeam } from "../types/cup";

export interface CreateCupTourData {
  number: number;
  name: string | null;
...
interface CreateCupTourModalProps {
  open: boolean;
  onClose: () => void;
  teams: CupGroupTeam[];
  onSubmit: (data: CreateCupTourData) => Promise<void>;
  groupName: string;
  nextTourNumber: number;
}

const CreateCupTourModal: FC<CreateCupTourModalProps> = ({
  open,
  onClose,
  teams,
  onSubmit,
  ...
  const [formData, setFormData] = useState<CreateCupTourData>({
  ...
  return (
    ...
    >
      ...
      >
        Добавить тур в группу "{groupName}"
        <IconButton
          onClick={handleClose}
          disabled={submitting}
          sx={{
...
export default CreateCupTourModal;
